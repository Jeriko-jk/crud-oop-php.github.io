	...
	public function updateData($nim, $nama, $data)
{
    ...
    if ($stmt = $this->prepare($sql)) {
        ...
        $param_nim = $nim;
        $param_nama = $nama;
        $param_data = $data; // Menetapkan nilai $data ke $param_data
        if ($stmt->execute()) {
            $stmt->close(); // Memindahkan stmt->close() ke dalam blok if
            return true;
        } else {
            $stmt->close(); // Memindahkan stmt->close() ke dalam blok if
            return false;
        ...
        return false; // Mengembalikan false jika prepare() gagal
    }
}
